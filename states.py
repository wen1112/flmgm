...
try:
    from FeatureCloud.app.engine.app import SMPCOperation, SMPCSerialization
except Exception:
    SMPCOperation = None
    SMPCSerialization = None
    ...
    if label_values:
        ...
        for metadata_file in metadata_paths:
            ...
            if label_column not in metadata_df.columns:
                # 报错提示
                raise ValueError(f"Label column '{label_column}' not found in metadata file '{metadata_file}'")
...
def flatten_state_dict(state_dict: Dict[str, torch.Tensor]) -> Tuple[Dict[str, List], List[float]]:
    keys: List[str] = []
    ...
    for key, value in state_dict.items():
        ...
        flat.extend(arr.reshape(-1).tolist())
...
def unflatten_state_dict(flat: List[float], meta: Dict[str, List]) -> Dict[str, torch.Tensor]:
    keys = meta["keys"]
    shapes = meta["shapes"]
    sizes = meta["sizes"]
    ...
    for key, shape, size in zip(keys, shapes, sizes):
        chunk = np.asarray(flat[offset:offset + size], dtype=np.float32).reshape(shape)
        state[key] = torch.from_numpy(chunk)
        offset += size
...
def collect_timing_series(timing_path: str, stage: str, key: str, role: str) -> Tuple[List[int], List[float]]:
    rounds: List[int] = []
    values: List[float] = []
...
def align_series_by_rounds(series_map: Dict[str, Tuple[List[int], List[float]]]) -> Tuple[List[int], Dict[str, List[float]]]:
    all_rounds: List[int] = []
    for rounds, _ in series_map.values():
    ...
    for name, (rounds, values) in series_map.items():
        lookup = {int(r): float(v) for r, v in zip(rounds, values)}
        aligned[name] = [lookup.get(r, float("nan")) for r in unique_rounds]

    return unique_rounds, aligned


def plot_timing_multiline(outputs_base_dir: str, filename: str, rounds: List[int], series: Dict[str, List[float]], title: str):
    if not rounds or not series:
        return None
    return plot_metrics(outputs_base_dir, filename, rounds, series, title, "Seconds")


def plot_timing_stacked_area(outputs_base_dir: str, filename: str, rounds: List[int], series: Dict[str, List[float]], title: str):
    if not rounds or not series:
        return None
    plots_dir = os.path.join(outputs_base_dir, "plots")
    ensure_dir(plots_dir)
    ...
    values = [np.nan_to_num(series[label], nan=0.0) for label in labels]

    plt.figure(figsize=(10, 6))
    plt.stackplot(rounds, values, labels=labels, alpha=0.85)
    plt.xlabel("Round")
    plt.ylabel("Seconds")
    plt.title(title)
    plt.grid(True, alpha=0.3)
...
def plot_timing_grouped_bar(outputs_base_dir: str, filename: str, rounds: List[int], series: Dict[str, List[float]], title: str):
    if not rounds or not series:
        return None
    plots_dir = os.path.join(outputs_base_dir, "plots")
    ensure_dir(plots_dir)
    ...
    width = min(0.8 / max(num_series, 1), 0.2)

    plt.figure(figsize=(12, 6))
    for idx, label in enumerate(labels):
        values = np.nan_to_num(series[label], nan=0.0)
    ...
    plt.ylabel("Seconds")
    plt.title(title)
    plt.xticks(x, rounds, rotation=45)
...
def safe_send_to_coordinator(
    app_state: AppState,
    payload: Dict,
    use_dp: bool,
    use_smpc: bool,
    memo: str,
    strict_smpc: bool = True,
):
    try:
        ...
        if use_smpc:
            msg = (
                f"[{now_ts()}] [privacy] SMPC fallback in send_data_to_coordinator "
                f"(memo={memo}): runtime does not accept use_smpc. error={exc}"
            )
            app_state.log(msg)
            if strict_smpc:
                raise RuntimeError(f"{msg}; strict_smpc=True blocks non-SMPC fallback") from exc
            app_state.log(
                f"[{now_ts()}] [privacy] strict_smpc=False, fallback to non-SMPC send_data_to_coordinator"
            )
...
def safe_await_data(
    ...
    try:
        ...
        if use_smpc:
            msg = (
                f"[{now_ts()}] [privacy] SMPC fallback in await_data "
                f"(memo={memo}): runtime does not accept use_smpc. error={exc}"
            )
            app_state.log(msg)
            if strict_smpc:
                raise RuntimeError(f"{msg}; strict_smpc=True blocks non-SMPC fallback") from exc
            app_state.log(
                f"[{now_ts()}] [privacy] strict_smpc=False, fallback to non-SMPC await_data"
            )
        return app_state.await_data(use_dp=use_dp, memo=memo)


def safe_gather_data(
    app_state: AppState,
    use_dp: bool,
    use_smpc: bool,
    memo: str,
    ...
    try:
        ...
        if use_smpc:
            msg = (
                f"[{now_ts()}] [privacy] SMPC fallback in gather_data "
                f"(memo={memo}): runtime does not accept use_smpc. error={exc}"
            )
            app_state.log(msg)
            if strict_smpc:
                raise RuntimeError(f"{msg}; strict_smpc=True blocks non-SMPC fallback") from exc
            app_state.log(
                f"[{now_ts()}] [privacy] strict_smpc=False, fallback to non-SMPC gather_data"
            )
        return app_state.gather_data(use_dp=use_dp, memo=memo)


def safe_broadcast_data(
    app_state: AppState,
    payload: Dict,
    use_dp: bool,
    use_smpc: bool,
    ...
    try:
        ...
        if use_smpc:
            msg = (
                f"[{now_ts()}] [privacy] SMPC fallback in broadcast_data "
                f"(memo={memo}): runtime does not accept use_smpc. error={exc}"
            )
            app_state.log(msg)
            if strict_smpc:
                raise RuntimeError(f"{msg}; strict_smpc=True blocks non-SMPC fallback") from exc
            app_state.log(
                f"[{now_ts()}] [privacy] strict_smpc=False, fallback to non-SMPC broadcast_data"
            )
        app_state.broadcast_data(payload, send_to_self=True, use_dp=use_dp, memo=memo)


def uplink_memo(round_idx: int) -> str:
    return f"uplink-round-{int(round_idx)}"


def downlink_memo(round_idx: int) -> str:
    return f"downlink-round-{int(round_idx)}"
...
class InitialState(AppState):
    ...
    def run(self):
        ...
        if cfg.get("use_smpc"):
            ...
            if SMPCOperation is not None and SMPCSerialization is not None:
                self.configure_smpc(
                    exponent=exponent,
                    shards=shards,
                ...
                self.log(
                    f"[{now_ts()}] [initial] SMPC enabled "
                    f"(operation=ADD, serialization=JSON, exponent={exponent}, shards={shards})"
                )
            else:
                self.log(
                    f"[{now_ts()}] [initial] SMPC enums unavailable in current SDK; "
                    "falling back to configure_smpc(exponent, shards)"
                )
                self.configure_smpc(
                    exponent=exponent,
                    shards=shards,
                ...
                self.log(
                    f"[{now_ts()}] [initial] SMPC enabled "
                    f"(operation/serialization from SDK default, exponent={exponent}, shards={shards})"
                )
        ...
        smpc_state_meta, _ = flatten_state_dict(global_state)
        ...
        self.store("smpc_state_meta", smpc_state_meta)
        self.store("round", 0)
        # 全局早停初始化
...
class LocalTrainState(AppState):
    ...
    def run(self):
        ...
        strict_smpc = bool(cfg.get("strict_smpc", True))
        smpc_allow_list_fallback = bool(cfg.get("smpc_allow_list_fallback", True))
        if use_smpc:
            _, state_vec = flatten_state_dict(local_state)
            weight = float(len(train_set))
            self.log(
                f"[{now_ts()}] [local_train] Round {round_idx}: SMPC vec_len={len(state_vec)} weight={weight}"
            )
            # SMPC payload must contain numeric values only.
            payload = {
                "weight": weight,
                "state_vec": [v * weight for v in state_vec],
            }
        t3 = time.perf_counter()
        self.log(f"[{now_ts()}] [local_train] Round {round_idx}: sending update to coordinator")
        safe_send_to_coordinator(
            self,
            payload,
            use_dp=use_dp,
            use_smpc=use_smpc,
            memo=uplink_memo(round_idx),
            strict_smpc=strict_smpc,
        )
...
class WaitGlobalState(AppState):
    ...
    def run(self):
        ...
        payload = safe_await_data(
            self,
            use_dp=bool(cfg.get("use_dp", False)),
            # Global model delivery does not require SMPC; SMPC is only used on uplink aggregation.
            use_smpc=False,
            memo=downlink_memo(round_idx + 1),
        )
    ...
    def run(self):
        ...
        if use_smpc:
            if isinstance(data_list, list):
                if len(data_list) == 1:
                    # Some SDK/runtime variants wrap aggregated SMPC payload as a one-item list.
                    self.log(
                        f"[{now_ts()}] [privacy] SMPC gather returned one-item list; unwrapping payload"
                    )
                    smpc_payload = data_list[0]
                elif strict_smpc and not smpc_allow_list_fallback:
                    raise ValueError(
                        "SMPC gather returned per-client list; set smpc_allow_list_fallback=true "
                        "or strict_smpc=false to allow list fallback aggregation"
                    )
                else:
                    self.log(
                        f"[{now_ts()}] [privacy] SMPC list fallback enabled: aggregating "
                        f"{len(data_list)} list items on coordinator"
                    )
                    if not data_list:
                        raise ValueError("SMPC gather returned empty list")
                    first_vec = data_list[0].get("state_vec", []) if data_list else []
                    summed_vec = [0.0 for _ in range(len(first_vec))]
                    total_weight = 0.0
                    for idx, item in enumerate(data_list):
                        vec = item.get("state_vec", [])
                        if len(vec) != len(summed_vec):
                            raise ValueError(f"SMPC vector length mismatch at list item {idx}")
                        summed_vec = [a + b for a, b in zip(summed_vec, vec)]
                        total_weight += float(item.get("weight", 0.0))
                    smpc_payload = {"state_vec": summed_vec, "weight": total_weight}
            else:
                smpc_payload = data_list

            state_meta = self.load("smpc_state_meta")
            summed_vec = smpc_payload.get("state_vec", [])
            total_weight = float(smpc_payload.get("weight", 0.0))

            if not state_meta:
                raise ValueError("SMPC state_meta missing")
            if total_weight <= 0:
                raise ValueError("SMPC total_weight must be positive")

            avg_vec = [v / total_weight for v in summed_vec]
            self.log(
                f"[{now_ts()}] [aggregate] Round {round_idx}: SMPC vec_len={len(avg_vec)} total_weight={total_weight}"
            )
            global_state = unflatten_state_dict(avg_vec, state_meta)
            agg_time = 0.0
            self.store("global_state", global_state)
        else:
        ...
        if self.is_coordinator and cfg.get("global_val_file") and cfg.get("global_val_metadata_file"):
            try:
                ...
                timing_path = os.path.join(outputs_base, "metrics", "timing.jsonl")
                timing_defs = [
                    ("local_data", "local_train", "data_time_sec"),
                    ("local_train", "local_train", "train_time_sec"),
                    ("local_eval", "local_train", "eval_time_sec"),
                    ("agg_gather", "aggregate", "gather_time_sec"),
                    ("agg_aggregate", "aggregate", "aggregate_time_sec"),
                    ("global_val", "aggregate", "global_val_time_sec"),
                    ("wait_global", "wait_global", "wait_time_sec"),
                ]

                for role in ["coordinator", "participant"]:
                    series_map: Dict[str, Tuple[List[int], List[float]]] = {}
                    for label, stage, key in timing_defs:
                        rounds_i, values_i = collect_timing_series(timing_path, stage, key, role)
                        if values_i:
                            series_map[label] = (rounds_i, values_i)

                    rounds_all, series_all = align_series_by_rounds(series_map)
                    if not rounds_all or not series_all:
                        continue

                    plot_timing_multiline(
                        outputs_base,
                        f"timing_overview_lines_{role}.png",
                        rounds_all,
                        series_all,
                        f"Timing Overview ({role})",
                    )
                    plot_timing_stacked_area(
                        outputs_base,
                        f"timing_overview_stack_{role}.png",
                        rounds_all,
                        series_all,
                        f"Timing Overview Stacked ({role})",
                    )
                    plot_timing_grouped_bar(
                        outputs_base,
                        f"timing_overview_bars_{role}.png",
                        rounds_all,
                        series_all,
                        f"Timing Overview Bars ({role})",
                    )

                combined_series_map: Dict[str, Tuple[List[int], List[float]]] = {}
                for role in ["coordinator", "participant"]:
                    for label, stage, key in timing_defs:
                        rounds_i, values_i = collect_timing_series(timing_path, stage, key, role)
                        if values_i:
                            combined_series_map[f"{role}_{label}"] = (rounds_i, values_i)

                rounds_all, series_all = align_series_by_rounds(combined_series_map)
                if rounds_all and series_all:
                    plot_timing_multiline(
                        outputs_base,
                        "timing_overview_lines_all.png",
                        rounds_all,
                        series_all,
                        "Timing Overview (all roles)",
                    )
                    plot_timing_stacked_area(
                        outputs_base,
                        "timing_overview_stack_all.png",
                        rounds_all,
                        series_all,
                        "Timing Overview Stacked (all roles)",
                    )
                    plot_timing_grouped_bar(
                        outputs_base,
                        "timing_overview_bars_all.png",
                        rounds_all,
                        series_all,
                        "Timing Overview Bars (all roles)",
                    )
            except Exception as exc:
